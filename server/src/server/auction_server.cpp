...
Auction_server::Auction_server(const std::string &ip, int port, const std::string &route) {
    ...
    server.Get(R"(/pending/(\d+))", [&](const Request& req, Response& res){db.getPendingRatings(req, res);});
    server.Get(R"(/ratings/(\d+))",[&](const Request& req, Response& res) {db.getAllRatings(req,res);});
